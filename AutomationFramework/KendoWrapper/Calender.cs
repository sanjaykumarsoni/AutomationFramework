...
{
    /// <summary>
    /// To automate kendo calender. Not in use : future considerations.
    /// </summary>
    ...
    public class Calendar : KendoWidget
    {
        private IJavaScriptExecutor driver;
        public Calendar(IWebElement element, By by, string name)
            : base(element, by, "kendoCalendar")
        {
        ...
        {
            get { return "kendoCalendar"; }
        }

        private const int WeeksPerPage = 6;
        /// <summary>
        /// This method is temporary written to click the date. it will not be helping in all type of calender.
        /// </summary>
        /// <param name="date"></param>
        public void ClickDate(String date)
        {
            IWebElement table = Driver.FindElement(By.CssSelector("k-content"), 5);
            ReadOnlyCollection<IWebElement> tableRows = table.FindElements(By.XPath("//tr"));
            foreach (IWebElement row in tableRows)
            {
                ReadOnlyCollection<IWebElement> cells = row.FindElements(By.XPath("td"));
        ...
        {
            var script = "$(arguments[0]).kendoCalendar('kendoCalendar').view().name";
            string View = (string)this.driver.ExecuteScript(script, FindElement());
            var view = View;
            if (view == MonthView)
            {
        ...
        {
            string View = (string)this.driver.ExecuteScript(Scripts.Calendar_view_name, FindElement());
            var view = View;
            if (view == MonthView)
            {
        ...
        ///// </summary>
        //public string View => (string)Driver.ExecuteScript(Script.Calendar_view_name, FindElement());
        ...
        {
            var value = (long)this.driver.ExecuteScript(Scripts.Calendar_options_animation_vertical_duration, FindElement());
            return Convert.ToInt32(value);
        }

        private int NextPrevAnimationTime()
        {
            var value = (long)this.driver.ExecuteScript(Scripts.Calendar_options_animation_horizontal_duration, FindElement());
            return Convert.ToInt32(value);
        }
    }
